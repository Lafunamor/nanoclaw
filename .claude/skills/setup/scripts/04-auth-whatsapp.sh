...
EOF
    ...
    # Allow up to 90s: whatsapp-auth.ts may retry on 405 with up to ~2min total backoff
    log "Polling for pairing code (90s timeout)"
    PAIRING_CODE=""
      ...
      if [ -f "$PROJECT_ROOT/store/auth-status.txt" ]; then
        ...
        case "$STATUS_CONTENT" in
          ...
          pairing_code:*)
            ...
            ;;
          failed:*)
            # Only bail early on hard failures (not 405, which the script retries)
            case "$STATUS_CONTENT" in
              failed:405) : ;; # 405 is being retried by whatsapp-auth.ts — keep waiting
              *)
